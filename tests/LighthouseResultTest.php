...
it('can save HAR file when HAR data is available', function () {
    $tempDirectory = (new TemporaryDirectory(getTemporaryDirectoryPath()))->empty();
    $harPath = $tempDirectory->path('report.har');

    $rawResult = getJsonStub('example-com-result');
    $rawResult['artifacts']['DevtoolsLog'] = [
        [
    ...
    $lighthouseResult = new LighthouseResult($rawResult);
    $lighthouseResult->saveHar($harPath);

    expect(file_exists($harPath))->toBeTrue();

    $harContent = json_decode(file_get_contents($harPath), true);
    expect($harContent)->toHaveKeys(['log']);
    expect($harContent['log'])->toHaveKeys(['version', 'creator', 'entries']);
    expect($harContent['log']['version'])->toBe('1.2');
    expect($harContent['log']['creator']['name'])->toBe('Lighthouse');
});

it('throws exception when trying to save HAR file without HAR data', function () {
    $tempDirectory = (new TemporaryDirectory(getTemporaryDirectoryPath()))->empty();
    $harPath = $tempDirectory->path('report.har');

    $this->lighthouseResult->saveHar($harPath);
})->throws(Exception::class, 'HAR data not available');
